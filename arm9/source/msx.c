...
void msx_slot_map_msx2_typeB(unsigned char Value)
{
    ...
    switch ((Value>>2) & 0x03)  // [0x4000~0x7FFF]
    {
        ...
        case 0x03:  // Slot 3:  Maps to 64K of RAM
            bCartInSegment[1] = 0;
            bRAMInSegment[1] = 1;
            MemoryMap[2] = (u8 *)RAM_Memory+0x4000;
            MemoryMap[3] = (u8 *)RAM_Memory+0x6000;
            break;
    }
...
ITCM_CODE void cpu_writeport_msx(register unsigned short Port,register unsigned char Value) 
{
    ...
    else if (Port >= 0xFC && Port <= 0xFF) // Memory Map
    {
        //debug[Port-0xFC] = Value;
        debug[15]++;
    }
}
...
void MSX_InitialMemoryLayout(u32 romSize)
{
    ...
    else if ((romSize >= (16 * 1024)) && (romSize <= (MAX_CART_SIZE * 1024)))   // We'll take anything between these two...
    {
        ...
        else if (mapperType == XEVIOUS)
        {
            ...
            MSXCartPtr[2] = (u8*)ROM_Memory+0x0000;        // Segment 0 default
            MSXCartPtr[3] = (u8*)ROM_Memory+0x2000;        // Segment 0 default
            MSXCartPtr[4] = (u8*)BIOS_Memory+0x8000;       // Segment Unmapped
            MSXCartPtr[5] = (u8*)BIOS_Memory+0x8000;       // Segment Unmapped
            MSXCartPtr[6] = (u8*)BIOS_Memory+0x8000;       // Segment Unmapped
            MSXCartPtr[7] = (u8*)BIOS_Memory+0x8000;       // Segment Unmapped
        }
