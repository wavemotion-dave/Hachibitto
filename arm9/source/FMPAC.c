...
#define FMPAC_RELEASE_STEP      (3984*2)        // RELEASE rate: 16.16 fixed-point step targeting a ~150ms
...
#define FMPAC_NOISE_OUT_SHIFT   10      // noise drums get MORE attenuation than melodic channels -
                            ...
                            // doesn't attempt. Kept at the same 4x-quieter-than-melodic
                            // ratio that measurably cut noise-active high-frequency
                            // energy roughly in half against real game data (Aleste).
...
void FMPACMixer(int len, s16 *dest, FMPAC *chip)
{
    ...
    for (i = 0; i < len; i++)
    {
        ...
        s32 mixed = ((s32)dest[i] + 32767) + sample; //TODO: the +32767 is needed when mixing with the AY. Find out why!
